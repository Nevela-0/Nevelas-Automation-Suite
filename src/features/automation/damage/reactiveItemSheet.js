...
async function runReactiveSheetStage(_stageName, sheet, root, fn) {
  try {
    const result = await fn();
    disableReactiveSheetControlsForLockedCompendium(sheet?.item, root);
    return result;
  } catch (_error) {
    return null;
  }
...
function lockedCompendiumForItem(item) {
  const packId = item?.pack || item?.collection?.metadata?.id || "";
  const pack = item?.compendium ?? (packId ? game?.packs?.get?.(packId) : null) ?? null;
  if (pack?.locked === true) return pack;
  if (item?.collection?.locked === true && item?.collection?.metadata?.id) return item.collection;
  return null;
}

function isItemInLockedCompendium(item) {
  return Boolean(lockedCompendiumForItem(item));
}

function disableReactiveSheetControlsForLockedCompendium(item, root) {
  if (!isItemInLockedCompendium(item) || !root?.querySelectorAll) return;
  const selector = [
    ".nas-onhit-effects",
    ".nas-onstruck-effects",
    ".nas-absorption-effects",
  ...
  for (const section of root.querySelectorAll(selector)) {
    section.dataset.nasLockedCompendium = "true";
    for (const control of section.querySelectorAll("input, select, textarea, button")) {
      control.disabled = true;
      control.title ||= "This item is in a locked compendium.";
    }
  }
}
...
function normalizeTemporaryHpDepletion(raw = {}) {
  const value = raw && typeof raw === "object" ? raw : {};
  const units = String(value.lockout?.units ?? "hour");
  return {
...
function normalizeBuffSaveOverrideConfig(raw = {}) {
  const value = raw && typeof raw === "object" ? raw : {};
  return {
    override: value.override === true || normalizeBuffSaveHandlingMode(value.mode) !== "ignore" || normalizeBuffSaveAlliesBypassMode(value.alliesBypass) !== "setting",
...
function onHitModeOptionsHtml(selected, { healing = false } = {}) {
  ...
  return options.map(([id, label]) =>
    `<option value="${escapeHtmlForSheet(id)}" ${id === current ? "selected" : ""}>${escapeHtmlForSheet(label)}</option>`
  ).join("");
}
...
function fortificationOptionsHtml(selected) {
  ...
  return getGrantedDefenseOptions("fortification").map((option) =>
    `<option value="${escapeHtmlForSheet(option.id)}" ${option.id === current ? "selected" : ""}>${escapeHtmlForSheet(option.label)}</option>`
  ).join("");
}
...
function syncReactiveSectionCollapsedChrome(section, expanded) {
  ...
  if (!header) return;
  const icon = header.querySelector("[data-nas-section-chevron]");
  if (icon) {
...
function reactiveSectionHeaderHtml(label, { expanded = false } = {}) {
  return `
    <h3 class="form-header nas-reactive-section-header" style="display:flex;align-items:center;gap:5px;">
      <span>${label}</span>
      <button type="button" data-nas-section-toggle title="${localize("toggleSection")}" style="display:inline-flex;align-items:center;justify-content:center;width:auto;min-width:0;max-width:none;height:auto;margin:0;padding:0 2px;border:0;background:transparent;box-shadow:none;line-height:1;cursor:pointer;">
        <i class="fas ${expanded ? "fa-chevron-down" : "fa-chevron-right"}" data-nas-section-chevron inert></i>
      </button>
    </h3>
  `;
}

function reactiveSectionClearButtonHtml() {
  return `
    <div class="form-group nas-reactive-clear-row" style="justify-content:flex-end;">
      <button type="button" data-nas-section-clear style="flex:0 0 auto;width:auto;min-width:0;">
        ${localize("clearSettings")}
      </button>
    </div>
  `;
}

function setReactiveSectionExpanded(section, expanded) {
  if (!section) return;
  section.dataset.nasExpanded = expanded ? "true" : "false";
  syncReactiveSectionCollapsedChrome(section, expanded);
}

function getReactiveSectionExpanded(sheetAppId, appKey, defaultExpanded = false) {
  const key = `${appKey}:expanded`;
  const stored = ReactiveUiState.get(sheetAppId, key);
  return typeof stored === "boolean" ? stored : defaultExpanded === true;
}

function bindReactiveSectionChrome(sheetAppId, appKey, section, { initiallyExpanded = false, onClear } = {}) {
  setReactiveSectionExpanded(section, getReactiveSectionExpanded(sheetAppId, appKey, initiallyExpanded));
  section.querySelector("[data-nas-section-toggle]")?.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    const expanded = section.dataset.nasExpanded !== "true";
    ReactiveUiState.set(sheetAppId, `${appKey}:expanded`, expanded);
    setReactiveSectionExpanded(section, expanded);
  });
  section.querySelector("[data-nas-section-clear]")?.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    onClear?.();
    ReactiveUiState.set(sheetAppId, `${appKey}:expanded`, false);
    setReactiveSectionExpanded(section, false);
  });
}
...
function attachDamageTypeMultiField(section, item, state, onChange) {
  ...
  const renderTags = () => {
    ...
    tags.innerHTML =
      labels.length > 0
        ? labels.map((label) => `<li class="tag">${escapeHtmlForSheet(label)}</li>`).join("")
...
function renderRuleTags(tags, optionList, selectedIds) {
  ...
  tags.innerHTML = ids
    .map((id) => optionList.find((option) => option.id === id)?.label ?? id)
    .sort((a, b) => a.localeCompare(b))
    .map((label) => `<li class="tag">${escapeHtmlForSheet(label)}</li>`)
    .join("");
}
      ...
      <div class="form-group">
        ...
        <div class="form-fields">
          <select data-rule-key="weaponType">
            ${getAbsorptionWeaponTypeOptions().map((option) => `<option value="${escapeHtmlForSheet(option.id)}">${escapeHtmlForSheet(option.label)}</option>`).join("")}
          </select>
        </div>
      </div>
      <div class="form-group">
      ...
      <div class="form-group" data-rule-row="convertTo">
        ...
        <div class="form-fields">
          <select data-rule-key="convertToDamageType">
            ${getDamageTypeOptions().map((option) => `<option value="${escapeHtmlForSheet(option.id)}">${escapeHtmlForSheet(option.label)}</option>`).join("")}
          </select>
        </div>
      </div>
  ...
  host.innerHTML = state.damageRules.map((rule, index) => {
    ...
    <div class="nas-onstruck-rule" data-nas-onstruck-rule="${index}" style="margin:0 0 6px;">
      ...
      <div class="form-group" data-rule-row="formula">
        ...
        <div class="form-fields"><input class="formula roll" type="text" data-rule-key="formula" value="${escapeHtmlForSheet(rule.formula ?? "")}" placeholder="${localize("formulaPlaceholder")}"></div>
      </div>
      <div class="form-group">
      ...
      <div class="form-group">
        ...
        <div class="form-fields">
          <select data-rule-key="sourceKind">
            ${sourceOptions.map((option) => `<option value="${option.id}">${escapeHtmlForSheet(option.label)}</option>`).join("")}
          </select>
        </div>
      </div>
      <div class="form-group">
      ...
      <div data-rule-row="save">
        ...
        <div class="form-group">
          ...
          <div class="form-fields"><input class="formula roll" type="text" data-rule-key="saveDcFormula" value="${escapeHtmlForSheet(rule.save?.dcFormula ?? "")}" placeholder="10 + @cl"></div>
        </div>
        <div class="form-group">
      ...
      <div data-rule-row="pool">
        <div class="form-group">
          ...
          <div class="form-fields">
            <input class="formula roll" type="text" data-rule-key="poolTotalFormula" value="${escapeHtmlForSheet(state.pool?.totalFormula ?? "")}" placeholder="min(50, 5 * @cl)" title="${localize("onStruckPoolTotalHint")}">
          </div>
        </div>
        <div class="form-group stacked">
          <label class="checkbox">
...
function toAbsorptionPayload(state) {
  ...
  if (!isAbsorptionConfigured(state)) return null;
  return {
    preset,
...
function reactiveRowsConfigured(rows) {
  return (Array.isArray(rows) ? rows : []).some((row) =>
    row?.enabled !== false
    && Array.isArray(row?.selectedIds)
    && row.selectedIds.some((id) => String(id ?? "").trim())
  );
}

function resistanceConfigured(resistance) {
  return Boolean(
    String(resistance?.custom ?? "").trim()
    || (Array.isArray(resistance?.value) && resistance.value.some((entry) =>
...
function isOnHitConfigured(state) {
  const normalized = normalizeOnHitConfig(state);
  return normalized.onHitFunction !== "none"
    || reactiveRowsConfigured(normalized.buffRows)
    || reactiveRowsConfigured(normalized.conditionRows);
}

function isOnStruckConfigured(state) {
  const normalized = normalizeOnStruckConfig(state);
  return normalized.onStruckFunction !== "none"
    || reactiveRowsConfigured(normalized.buffRows)
    || reactiveRowsConfigured(normalized.conditionRows);
}

function isAbsorptionConfigured(state) {
  if (!state || typeof state !== "object") return false;
  const normalized = normalizeAbsorptionConfig(state);
  const defaults = absorptionPresetDefaults(normalized.preset, normalized);
  return Boolean(
    state.enabled === true
    || (state.remaining != null && Number.isFinite(Number(state.remaining)))
    || (state.capacity != null && Number.isFinite(Number(state.capacity)))
    || normalized.preset !== "ablativeBarrier"
    || normalized.energyType !== normalizeAbsorptionPresetEnergyType(normalized.preset, defaults.energyType)
    || (String(state.totalFormula ?? "").trim() && String(state.totalFormula).trim() !== String(defaults.totalFormula))
    || (String(state.perAttackFormula ?? "").trim() && String(state.perAttackFormula).trim() !== String(defaults.perAttackFormula))
    || (normalized.preset === "custom" && Array.isArray(state.rules) && state.rules.length > 0)
  );
}

function isGrantedDefensesConfigured(state) {
  const normalized = normalizeGrantedDefenses(state);
  return resistanceConfigured(normalized.dr)
    || resistanceConfigured(normalized.eres)
    || (Array.isArray(normalized.di) && normalized.di.length > 0)
    || (Array.isArray(normalized.ci) && normalized.ci.length > 0)
    || (Array.isArray(normalized.dv) && normalized.dv.length > 0)
    || normalized.fortification !== "none"
    || normalized.criticalImmunity === true;
}

function isTemporaryHpConfigured(state) {
  const normalized = normalizeTemporaryHpConfig(state);
  return Boolean(
    String(normalized.formula ?? "").trim()
    || Number(normalized.max) > 0
    || normalized.regeneration?.enabled === true
    || String(normalized.regeneration?.formula ?? "").trim()
    || normalized.depletion?.mode !== "none"
    || normalized.depletion?.lockout?.enabled === true
  );
}

function isAppliedBuffOverrideEntryConfigured(entry) {
  if (!entry || entry.enabled === false) return false;
  const tempHp = entry.temporaryHp ?? {};
  const defenses = entry.grantedDefenses ?? {};
  const upkeep = entry.chargeUpkeep ?? {};
  return Boolean(
    String(entry.buffUuid ?? "").trim()
    || tempHp.enabled === true
    || String(tempHp.formula ?? "").trim()
    || tempHp.regeneration?.enabled === true
    || String(tempHp.regeneration?.formula ?? "").trim()
    || tempHp.deactivateAtZero === true
    || tempHp.lockout?.enabled === true
    || defenses.criticalImmunity === true
    || upkeep.enabled === true
  );
}

function isAppliedBuffOverridesConfigured(state) {
  const normalized = normalizeAppliedBuffOverrides(state);
  return (normalized.entries ?? []).some((entry) => isAppliedBuffOverrideEntryConfigured(entry));
}

function isBuffSaveGateConfigured(state) {
  return Boolean(
    String(state?.mode ?? "ignore") !== "ignore"
    || String(state?.alliesBypass ?? "setting") !== "setting"
  );
}
...
function toGrantedDefensesPayload(state) {
  ...
  return {
    ...
    criticalImmunity: normalized.criticalImmunity
  };
}

function toTemporaryHpPayload(state) {
  const normalized = normalizeTemporaryHpConfig(state);
  if (!isTemporaryHpConfigured(normalized)) return null;
...
function buildReactiveFlagUpdateData(flags = {}) {
  const rootPath = `flags.${MODULE.ID}.${REACTIVE_FLAG_KEY}`;
  const sanitizedFlags = deepClone(flags);
  const updateData = { [rootPath]: sanitizedFlags };
  for (const [key, value] of Object.entries({ ...sanitizedFlags })) {
    if (!isReactiveFlagDeletePayload(value)) continue;
    delete sanitizedFlags[key];
    updateData[`${rootPath}.-=${key}`] = null;
  }
  return updateData;
}
...
function scheduleFlagSave(item, key, updater) {
  ...
  if (isItemInLockedCompendium(item)) {
    SAVE_TIMEOUTS.delete(timeoutKey);
    return;
  }
  const nextTimer = setTimeout(async () => {
    SAVE_TIMEOUTS.delete(timeoutKey);
    if (isItemInLockedCompendium(item)) return;
    const current = getReactiveFlags(item);
    const next = updater(current);
    await item.update(buildReactiveFlagUpdateData(next), { render: false });
  }, 300);
  SAVE_TIMEOUTS.set(timeoutKey, nextTimer);
}

async function saveReactiveFlagsNow(item, key, updater) {
  if (!item || isItemInLockedCompendium(item)) return;
  const timeoutKey = `${item.uuid}:${key}`;
  const old = SAVE_TIMEOUTS.get(timeoutKey);
  if (old) clearTimeout(old);
  SAVE_TIMEOUTS.delete(timeoutKey);
...
async function renderBuffSaveGateSection(sheet, root) {
  ...
  section.innerHTML = `
    ${reactiveSectionHeaderHtml(escapeHtmlForSheet(localize("buffSaveGateHeader")), { expanded: isBuffSaveGateConfigured(state) })}
    <div data-nas-reactive-body>
    <div class="form-group" data-nas-row="mode">
      <label>${escapeHtmlForSheet(localize("buffSaveHandling"))}</label>
      <select data-nas-key="mode">${buffSaveHandlingOptionsHtml(state.mode)}</select>
    </div>
    <div class="form-group" data-nas-row="alliesBypass">
      <label>${escapeHtmlForSheet(localize("buffSaveAlliesBypass"))}</label>
      <select data-nas-key="alliesBypass">${buffSaveAlliesBypassOptionsHtml(state.alliesBypass)}</select>
    </div>
    ${reactiveSectionClearButtonHtml()}
    </div>
  `;

  const updateRows = () => {
    syncReactiveSectionCollapsedChrome(section, section.dataset.nasExpanded === "true");
  };
  const persist = () => {
    ReactiveUiState.set(sheet.appId, appKey, state);
  ...
  section.addEventListener("change", (event) => {
    ...
    if (!key) return;
    if (key === "mode") state.mode = normalizeBuffSaveHandlingMode(event.target.value);
  ...
  bindReactiveSectionChrome(sheet.appId, appKey, section, {
    initiallyExpanded: isBuffSaveGateConfigured(state),
    onClear: () => {
      Object.assign(state, normalizeBuffSaveOverrideConfig({}));
      ReactiveUiState.set(sheet.appId, appKey, state);
      void saveReactiveFlagsNow(item, appKey, (flags) => {
        flags[BUFF_SAVE_ACTION_SHEET_KEY] ??= {};
        flags[BUFF_SAVE_ACTION_SHEET_KEY][action.id] = toBuffSaveOverridePayload(state);
        return flags;
...
async function renderOnHitSection(sheet, root) {
  ...
  section.innerHTML = `
    ${reactiveSectionHeaderHtml(onHitHeaderLabel, { expanded: isOnHitConfigured(state) })}
    <div data-nas-reactive-body>
    ...
    <div class="form-group input-select duration" data-nas-row="lifestealTemporaryHpDuration">
      <label>${game.i18n.localize("PF1.Duration")}</label>
      <div class="form-fields">
        <input class="formula roll" type="text" data-nas-key="lifestealTemporaryHpDurationValue" value="${escapeHtmlForSheet(state.lifestealTemporaryHpDuration?.value ?? "1")}" placeholder="${game.i18n.localize("PF1.Formula")}">
  ...
  const updateRows = () => {
    ...
    const showPrimary = ["lifesteal", "grantTemporaryHp"].includes(fn);
  ...
  bindReactiveSectionChrome(sheet.appId, appKey, section, {
    initiallyExpanded: isOnHitConfigured(state),
    onClear: () => {
      const cleared = normalizeOnHitConfig({});
      Object.assign(state, cleared, { buffRows: [], conditionRows: [] });
      ReactiveUiState.set(sheet.appId, appKey, state);
      void saveReactiveFlagsNow(item, useActionSheetOverride ? `onhit-ov:${action.id}` : "onhit:item", (flags) => {
        if (useActionSheetOverride) {
          flags[ON_HIT_ACTION_SHEET_KEY] ??= {};
          flags[ON_HIT_ACTION_SHEET_KEY][action.id] = toOnHitPayload(state);
        } else {
...
async function renderOnStruckSection(sheet, root) {
  ...
  if (isAbsorptionConfigured(fromAbsorptionFlag) && String(state.onStruckFunction ?? "none") === "none" && item.type === "buff") {
  ...
  section.innerHTML = `
    ...
    <div class="form-group" data-nas-row="absorptionTotalFormula">
      ...
      <div class="form-fields">
        <input class="formula roll" type="text" data-nas-key="absorptionTotalFormula" value="${escapeHtmlForSheet(state.absorption.totalFormula ?? "")}" placeholder="min(50, 5 * @cl)" title="${localize("absorptionTotalFormulaHint")}">
      </div>
    </div>
    <div class="form-group" data-nas-row="absorptionPerAttackFormula">
      ...
      <div class="form-fields">
        <input class="formula roll" type="text" data-nas-key="absorptionPerAttackFormula" value="${escapeHtmlForSheet(state.absorption.perAttackFormula ?? "")}" placeholder="5" title="${localize("absorptionPerAttackFormulaHint")}">
      </div>
    </div>
  ...
  const syncEnergyTypeSelect = () => {
    ...
    select.innerHTML = allowed
      .map((id) => `<option value="${escapeHtmlForSheet(id)}">${escapeHtmlForSheet(titleCaseOptionLabel(id, labels.find((option) => option.id === id)?.label ?? id))}</option>`)
      .join("");
  ...
  const updateRows = () => {
    ...
    const showValue = damageLike && state.mode === "percentOfFinalDamage";
    const showFormula = damageLike && state.mode === "formula";
    const showMode = damageLike;
    const showDamageTypes = false;
    ...
    if (onStruckRules) onStruckRules.style.display = onStruckDamageLike ? "" : "none";
    for (const row of section.querySelectorAll('[data-nas-row^="absorption"]')) {
      row.style.display = absorptionLike ? "" : "none";
    }
    const customRules = section.querySelector('[data-nas-row="absorptionCustomRules"]');
    if (customRules) customRules.style.display = absorptionLike && state.absorption.preset === "custom" ? "" : "none";
    const energyTypeRow = section.querySelector('[data-nas-row="absorptionEnergyType"]');
    if (energyTypeRow) energyTypeRow.style.display = absorptionLike && absorptionPresetUsesEnergyType(state.absorption.preset) ? "" : "none";
    ...
    if (totalFormulaRow) totalFormulaRow.style.display = absorptionLike && usesDischargeTotal ? "" : "none";
    const dischargeOption = section.querySelector('[data-nas-option="absorptionDischargeAtZero"]');
    if (dischargeOption) dischargeOption.style.display = absorptionLike && usesDischargeTotal ? "" : "none";
    const badgeOption = section.querySelector('[data-nas-option="absorptionShowBadge"]');
    if (badgeOption) badgeOption.style.display = absorptionLike && usesDischargeTotal ? "" : "none";
    const hpBarOption = section.querySelector('[data-nas-option="absorptionShowHpBar"]');
    if (hpBarOption) hpBarOption.style.display = absorptionLike && usesDischargeTotal ? "" : "none";
  };
  ...
  const saveOnStruckAndAbsorption = () => {
    scheduleFlagSave(item, "onstruck", (flags) => {
      ...
      const nextAbsorption = state.onStruckFunction === "damageAbsorption"
        ...
        : null;
      const resetRemaining = nextAbsorption != null && isAbsorptionConfigured(nextAbsorption) && (
        !isAbsorptionConfigured(previous)
      ...
      flags.absorption = nextAbsorption == null ? null : {
        ...nextAbsorption,
  ...
  bindReactiveSectionChrome(sheet.appId, appKey, section, {
    ...
    onClear: () => {
      ...
      Object.assign(state, cleared, {
        absorption: normalizeAbsorptionConfig({}),
        buffRows: [],
        conditionRows: []
      });
      ReactiveUiState.set(sheet.appId, appKey, state);
...
async function renderAbsorptionSection(sheet, root) {
  ...
  section.innerHTML = `
    ${reactiveSectionHeaderHtml(localize("absorptionHeader"), { expanded: isAbsorptionConfigured(state) })}
    <div data-nas-reactive-body>
      <div class="form-group">
        <label title="${localize("absorptionTotalFormulaHint")}">${localize("absorptionTotalFormula")}</label>
        <div class="form-fields">
          <input class="formula roll" type="text" data-nas-key="totalFormula" value="${escapeHtmlForSheet(state.totalFormula ?? "")}" placeholder="min(50, 5 * @cl)" title="${localize("absorptionTotalFormulaHint")}">
        </div>
      </div>
      <div class="form-group">
        <label title="${localize("absorptionPerAttackFormulaHint")}">${localize("absorptionPerAttackFormula")}</label>
        <div class="form-fields">
          <input class="formula roll" type="text" data-nas-key="perAttackFormula" value="${escapeHtmlForSheet(state.perAttackFormula ?? "")}" placeholder="5" title="${localize("absorptionPerAttackFormulaHint")}">
        </div>
      </div>
      <div class="form-group">
  ...
  const writeState = () => {
    ...
    scheduleFlagSave(item, "absorption", (flags) => {
      const previous = normalizeAbsorptionConfig(flags.absorption);
      const next = toAbsorptionPayload(state);
      flags.absorption = next == null ? null : {
        ...next,
  ...
  bindReactiveSectionChrome(sheet.appId, appKey, section, {
    initiallyExpanded: isAbsorptionConfigured(state),
    onClear: () => {
      Object.assign(state, normalizeAbsorptionConfig({}));
      ReactiveUiState.set(sheet.appId, appKey, state);
      void saveReactiveFlagsNow(item, "absorption", (flags) => {
        flags.absorption = toAbsorptionPayload(state);
        return flags;
      }).then(() => sheet?.render?.(false));
    }
  });
...
function attachGrantedDefenseResistanceField(section, item, state, key, onChange) {
  ...
  const renderTags = () => {
    ...
    for (const [index, entry] of entries.entries()) {
      ...
      li.innerHTML = `
        <span>${escapeHtmlForSheet(entryLabel)}</span>
        <input type="checkbox" data-nas-defense-stack-index="${index}" ${entry.stackable === true ? "checked" : ""}
          title="${localize("grantedDefenseStackable")}" aria-label="${escapeHtmlForSheet(`${entryLabel}: ${localize("grantedDefenseStackable")}`)}"
...
function makeGrantedDefenseWriteState(section, item, state, sheetAppId) {
  return ({ preserveResistanceState = false } = {}) => {
    ...
    state.criticalImmunity = section.querySelector('input[data-nas-key="criticalImmunity"]')?.checked === true;
...
async function renderGrantedDefensesSection(sheet, root) {
  ...
  section.innerHTML = `
    ${reactiveSectionHeaderHtml(localize("grantedDefensesHeader"), { expanded: isGrantedDefensesConfigured(state) })}
    <div data-nas-reactive-body>
      <div class="form-group">
      ...
      <div class="form-group stacked">
        <label class="checkbox">
          <input type="checkbox" data-nas-key="criticalImmunity" ${state.criticalImmunity ? "checked" : ""}>
          ${localize("grantedDefenseCriticalImmunity")}
        </label>
      </div>
      ${["di", "ci", "dv"].map((key) => `
        <div class="form-group">
  ...
  section.querySelector('input[data-nas-key="criticalImmunity"]')?.addEventListener("change", (event) => {
    excludeNasChangeFromParentForm(event);
    writeState();
  });
  ...
  bindReactiveSectionChrome(sheet.appId, appKey, section, {
    initiallyExpanded: isGrantedDefensesConfigured(state),
    onClear: () => {
      Object.assign(state, normalizeGrantedDefenses({}));
      ReactiveUiState.set(sheet.appId, appKey, state);
      void saveReactiveFlagsNow(item, "grantedDefenses", (flags) => {
        flags.grantedDefenses = toGrantedDefensesPayload(state);
        return flags;
      }).then(() => sheet?.render?.(false));
    }
  });
...
function itemSupportsAppliedBuffOverrides(item) {
  return Boolean(item && item.type !== "buff");
}

function newAppliedBuffOverrideEntry() {
  return {
    id: createNasId(),
    enabled: true,
    buffUuid: "",
    temporaryHp: {
      enabled: false,
      formula: "",
      regeneration: { enabled: false, formula: "" },
      deactivateAtZero: false,
      lockout: { enabled: false, value: "24", units: "hour" }
    },
    grantedDefenses: { criticalImmunity: false },
    chargeUpkeep: { enabled: false, costFormula: "1", intervalValue: "1", intervalUnits: "minute" }
  };
}

function toAppliedBuffOverridesPayload(state = {}) {
  const payload = normalizeAppliedBuffOverrides(state);
  delete payload.enabled;
  return payload;
}

async function renderAppliedBuffOverridesSection(sheet, root) {
  const detailsTab = findDetailsTab(root);
  if (!detailsTab) return;
  const item = sheet?.item;
  if (!itemSupportsAppliedBuffOverrides(item)) return;
  if (detailsTab.querySelector(".nas-applied-buff-overrides")) return;

  const appKey = "appliedBuffOverrides";
  const fromState = ReactiveUiState.get(sheet.appId, appKey);
  const fromFlag = normalizeAppliedBuffOverrides(getReactiveFlags(item).appliedBuffOverrides);
  const state = deepClone(fromState ?? fromFlag);
  if (!Array.isArray(state.entries)) state.entries = [];
  ReactiveUiState.set(sheet.appId, appKey, state);

  const section = document.createElement("div");
  section.classList.add("nas-applied-buff-overrides");
  section.innerHTML = `
    ${reactiveSectionHeaderHtml(localize("appliedBuffOverridesHeader"), { expanded: isAppliedBuffOverridesConfigured(state) })}
    <div data-nas-reactive-body>
      <div data-nas-applied-buff-rows></div>
      <p class="notes">${localize("appliedBuffOverridesHint")}</p>
      <button type="button" data-nas-add-applied-buff-override>
        <i class="fas fa-plus" inert></i> ${localize("appliedBuffOverrideAdd")}
      </button>
      ${reactiveSectionClearButtonHtml()}
    </div>
  `;
  ...
  const temporaryHp = detailsTab.querySelector(".nas-temporary-hp");
  const absorption = detailsTab.querySelector(".nas-absorption-effects");
  const onStruck = detailsTab.querySelector(".nas-onstruck-effects");
  const advanced = findAdvancedHeaderInTab(detailsTab);
  const anchor = grantedDefenses ?? temporaryHp ?? absorption ?? onStruck ?? advanced;
  if (anchor) detailsTab.insertBefore(section, anchor);
  else detailsTab.appendChild(section);

  const rowsEl = section.querySelector("[data-nas-applied-buff-rows]");
  ...
  const readRows = () => {
    state.entries = [...section.querySelectorAll("[data-nas-applied-buff-row]")].map((row) => ({
      id: row.dataset.nasAppliedBuffRow || createNasId(),
      enabled: row.querySelector('[data-nas-key="rowEnabled"]')?.checked !== false,
      buffUuid: String(row.querySelector('[data-nas-key="buffUuid"]')?.value ?? "").trim(),
      temporaryHp: {
        enabled: row.querySelector('[data-nas-key="temporaryHpEnabled"]')?.checked === true,
        formula: String(row.querySelector('[data-nas-key="temporaryHpFormula"]')?.value ?? "").trim(),
        regeneration: {
          enabled: row.querySelector('[data-nas-key="regenerationEnabled"]')?.checked === true,
          formula: String(row.querySelector('[data-nas-key="regenerationFormula"]')?.value ?? "").trim()
        },
        deactivateAtZero: row.querySelector('[data-nas-key="deactivateAtZero"]')?.checked === true,
        lockout: {
          enabled: row.querySelector('[data-nas-key="lockoutEnabled"]')?.checked === true,
          value: String(row.querySelector('[data-nas-key="lockoutValue"]')?.value ?? "24"),
          units: row.querySelector('[data-nas-key="lockoutUnits"]')?.value ?? "hour"
        }
      },
      grantedDefenses: {
        criticalImmunity: row.querySelector('[data-nas-key="criticalImmunity"]')?.checked === true
      },
      chargeUpkeep: {
        enabled: row.querySelector('[data-nas-key="chargeUpkeepEnabled"]')?.checked === true,
        costFormula: String(row.querySelector('[data-nas-key="chargeCostFormula"]')?.value ?? "1"),
        intervalValue: String(row.querySelector('[data-nas-key="chargeIntervalValue"]')?.value ?? "1"),
        intervalUnits: row.querySelector('[data-nas-key="chargeIntervalUnits"]')?.value ?? "minute"
      }
    }));
  };
  ...
  const persist = () => {
    ...
    ReactiveUiState.set(sheet.appId, appKey, state);
    updateRows();
    scheduleFlagSave(item, "appliedBuffOverrides", (flags) => {
      flags.appliedBuffOverrides = toAppliedBuffOverridesPayload(state);
      return flags;
    });
  };

  const renderRows = () => {
    rowsEl.innerHTML = state.entries.length ? state.entries.map((entry) => {
      const normalized = normalizeAppliedBuffOverrides({ enabled: true, entries: [entry] }).entries[0] ?? newAppliedBuffOverrideEntry();
      return `
        <div data-nas-applied-buff-row="${escapeHtmlForSheet(normalized.id)}" style="border:1px solid var(--color-border-light-primary,#999);border-radius:4px;padding:6px;margin:0 0 8px;">
          <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;margin-bottom:4px;">
            <label class="checkbox" style="margin:0;">
              <input type="checkbox" data-nas-key="rowEnabled" ${normalized.enabled ? "checked" : ""}>
              ${localize("appliedBuffOverrideEnabled")}
            </label>
            <button type="button" data-nas-remove-applied-buff-override="${escapeHtmlForSheet(normalized.id)}" title="${localize("removeRow")}">
              <i class="fas fa-trash" inert></i>
            </button>
          </div>
          <div class="form-group">
            <label>${localize("appliedBuffOverrideBuffUuid")}</label>
            <div class="form-fields">
              <input type="text" data-nas-key="buffUuid" value="${escapeHtmlForSheet(normalized.buffUuid)}" placeholder="Compendium.package.pack.Item.id">
            </div>
          </div>
          <div class="form-group stacked">
            <label class="checkbox"><input type="checkbox" data-nas-key="temporaryHpEnabled" ${normalized.temporaryHp.enabled ? "checked" : ""}> ${localize("temporaryHpHeader")}</label>
            <label class="checkbox"><input type="checkbox" data-nas-key="regenerationEnabled" ${normalized.temporaryHp.regeneration.enabled ? "checked" : ""}> ${localize("temporaryHpRegeneration")}</label>
            <label class="checkbox"><input type="checkbox" data-nas-key="criticalImmunity" ${normalized.grantedDefenses.criticalImmunity ? "checked" : ""}> ${localize("grantedDefenseCriticalImmunity")}</label>
          </div>
          <div class="form-group">
            <label>${localize("temporaryHpAmount")}</label>
            <div class="form-fields">
              <input class="formula roll" type="text" data-nas-key="temporaryHpFormula" value="${escapeHtmlForSheet(normalized.temporaryHp.formula)}" placeholder="30">
            </div>
          </div>
          <div class="form-group">
            <label>${localize("temporaryHpRegenerationFormula")}</label>
            <div class="form-fields">
              <input class="formula roll" type="text" data-nas-key="regenerationFormula" value="${escapeHtmlForSheet(normalized.temporaryHp.regeneration.formula)}" placeholder="6">
            </div>
          </div>
          <div class="form-group stacked">
            <label class="checkbox"><input type="checkbox" data-nas-key="deactivateAtZero" ${normalized.temporaryHp.deactivateAtZero ? "checked" : ""}> ${localize("temporaryHpDeactivateAtZero")}</label>
            <label class="checkbox"><input type="checkbox" data-nas-key="lockoutEnabled" ${normalized.temporaryHp.lockout.enabled ? "checked" : ""}> ${localize("temporaryHpLockout")}</label>
          </div>
          <div class="form-group input-select">
            <label>${localize("temporaryHpLockoutDuration")}</label>
            <div class="form-fields">
              <input class="formula roll" type="text" data-nas-key="lockoutValue" value="${escapeHtmlForSheet(normalized.temporaryHp.lockout.value)}" placeholder="24">
              <select data-nas-key="lockoutUnits">${timePeriodOptionsHtml(normalized.temporaryHp.lockout.units)}</select>
            </div>
          </div>
          <div class="form-group stacked">
            <label class="checkbox"><input type="checkbox" data-nas-key="chargeUpkeepEnabled" ${normalized.chargeUpkeep.enabled ? "checked" : ""}> ${localize("appliedBuffChargeUpkeep")}</label>
          </div>
          <div class="form-group">
            <label>${localize("appliedBuffChargeCost")}</label>
            <div class="form-fields">
              <input class="formula roll" type="text" data-nas-key="chargeCostFormula" value="${escapeHtmlForSheet(normalized.chargeUpkeep.costFormula)}" placeholder="1">
            </div>
          </div>
          <div class="form-group input-select">
            <label>${localize("appliedBuffChargeInterval")}</label>
            <div class="form-fields">
              <input class="formula roll" type="text" data-nas-key="chargeIntervalValue" value="${escapeHtmlForSheet(normalized.chargeUpkeep.intervalValue)}" placeholder="1">
              <select data-nas-key="chargeIntervalUnits">${timePeriodOptionsHtml(normalized.chargeUpkeep.intervalUnits)}</select>
            </div>
          </div>
        </div>
      `;
    }).join("") : `<p class="notes">${localize("appliedBuffOverrideNone")}</p>`;

    for (const control of rowsEl.querySelectorAll("input, select")) {
      control.addEventListener("change", (event) => {
        excludeNasChangeFromParentForm(event);
        persist();
      });
    }
    for (const button of rowsEl.querySelectorAll("[data-nas-remove-applied-buff-override]")) {
      button.addEventListener("click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        const id = event.currentTarget.dataset.nasRemoveAppliedBuffOverride;
  ...
  section.querySelector("[data-nas-add-applied-buff-override]")?.addEventListener("click", (event) => {
    event.preventDefault();
    event.stopPropagation();
    state.entries.push(newAppliedBuffOverrideEntry());
    renderRows();
    persist();
  });
  ...
  renderRows();
  bindReactiveSectionChrome(sheet.appId, appKey, section, {
    initiallyExpanded: isAppliedBuffOverridesConfigured(state),
    onClear: () => {
      state.entries = [];
      ReactiveUiState.set(sheet.appId, appKey, state);
      void saveReactiveFlagsNow(item, "appliedBuffOverrides", (flags) => {
        flags.appliedBuffOverrides = toAppliedBuffOverridesPayload(state);
        return flags;
      }).then(() => sheet?.render?.(false));
    }
...
async function renderTemporaryHpSection(sheet, root) {
  ...
  section.innerHTML = `
    ${reactiveSectionHeaderHtml(localize("temporaryHpHeader"), { expanded: isTemporaryHpConfigured(state) })}
    <div data-nas-reactive-body>
      <div class="form-group">
        <label>${localize("temporaryHpAmount")}</label>
        <div class="form-fields">
          <input class="formula roll" type="text" data-nas-key="formula" value="${escapeHtmlForSheet(state.formula ?? "")}" placeholder="${localize("formulaPlaceholder")}">
        </div>
      </div>
      <div class="form-group">
        <label>${localize("temporaryHpStacking")}</label>
      ...
      <div class="form-group stacked">
        <label class="checkbox">
          <input type="checkbox" data-nas-key="regenerationEnabled" ${state.regeneration?.enabled ? "checked" : ""}>
          ${localize("temporaryHpRegeneration")}
        </label>
      </div>
      <div class="form-group" data-nas-row="temporaryHpRegenerationFormula">
        <label>${localize("temporaryHpRegenerationFormula")}</label>
        <div class="form-fields">
          <input class="formula roll" type="text" data-nas-key="regenerationFormula" value="${escapeHtmlForSheet(state.regeneration?.formula ?? "")}" placeholder="5">
        </div>
      </div>
      <div class="form-group stacked">
        <label class="checkbox">
          <input type="checkbox" data-nas-key="deactivateAtZero" ${state.depletion?.mode === "deactivateSource" ? "checked" : ""}>
          ${localize("temporaryHpDeactivateAtZero")}
        </label>
        <label class="checkbox">
          <input type="checkbox" data-nas-key="lockoutEnabled" ${state.depletion?.lockout?.enabled ? "checked" : ""}>
          ${localize("temporaryHpLockout")}
        </label>
      </div>
      <div class="form-group input-select" data-nas-row="temporaryHpLockout">
        <label>${localize("temporaryHpLockoutDuration")}</label>
        <div class="form-fields">
          <input class="formula roll" type="text" data-nas-key="lockoutValue" value="${escapeHtmlForSheet(state.depletion?.lockout?.value ?? "24")}" placeholder="24">
          <select data-nas-key="lockoutUnits">
            ${timePeriodOptionsHtml(state.depletion?.lockout?.units ?? "hour")}
          </select>
        </div>
      </div>
      ${reactiveSectionClearButtonHtml()}
  ...
  const updateRows = () => {
    syncReactiveSectionCollapsedChrome(section, section.dataset.nasExpanded === "true");
    const regenRow = section.querySelector('[data-nas-row="temporaryHpRegenerationFormula"]');
    if (regenRow) regenRow.style.display = state.regeneration?.enabled ? "" : "none";
    const lockoutRow = section.querySelector('[data-nas-row="temporaryHpLockout"]');
    if (lockoutRow) lockoutRow.style.display = state.depletion?.lockout?.enabled ? "" : "none";
  };
  ...
  const writeState = () => {
    ...
    state.regeneration = normalizeTemporaryHpRegeneration({
      enabled: section.querySelector('input[data-nas-key="regenerationEnabled"]')?.checked === true,
      formula: String(section.querySelector('input[data-nas-key="regenerationFormula"]')?.value ?? "").trim()
    });
    state.depletion = normalizeTemporaryHpDepletion({
      mode: section.querySelector('input[data-nas-key="deactivateAtZero"]')?.checked === true ? "deactivateSource" : "none",
      lockout: {
        enabled: section.querySelector('input[data-nas-key="lockoutEnabled"]')?.checked === true,
        value: String(section.querySelector('input[data-nas-key="lockoutValue"]')?.value ?? "24"),
        units: section.querySelector('select[data-nas-key="lockoutUnits"]')?.value ?? "hour"
      }
    });
    ...
    scheduleFlagSave(item, "temporaryHp", (flags) => {
      ...
      const resetPool = next != null && isTemporaryHpConfigured(next) && (
        !isTemporaryHpConfigured(previous)
        || previous.formula !== next.formula
        || previous.max !== next.max
        || !Number.isFinite(Number(previous.remaining))
      );
      flags.temporaryHp = next == null ? null : {
        ...next,
  ...
  bindReactiveSectionChrome(sheet.appId, appKey, section, {
    initiallyExpanded: isTemporaryHpConfigured(state),
    onClear: () => {
      Object.assign(state, normalizeTemporaryHpConfig({}, item));
      ReactiveUiState.set(sheet.appId, appKey, state);
      void saveReactiveFlagsNow(item, "temporaryHp", (flags) => {
        flags.temporaryHp = toTemporaryHpPayload(state);
        return flags;
      }).then(() => sheet?.render?.(false));
    }
  });
...
function onRenderItemSheet(sheet, html) {
  ...
  void (async () => {
    ...
    await runReactiveSheetStage("grantedDefenses", sheet, root, () => renderGrantedDefensesSection(sheet, root));
    await runReactiveSheetStage("temporaryHp", sheet, root, () => renderTemporaryHpSection(sheet, root));
  })().finally(() => scheduleNasScrollRestoreRetry(sheet));
}
