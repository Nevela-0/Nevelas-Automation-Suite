...
export function registerDamageSettingsHandlebarsHelpers() {
    ...
    _damageSettingsHelpersRegistered = true;
...
function getActionSetting(item, actionId, settingPath, defaultValue = null) {
    const itemActionSettings = item.getFlag(MODULE.ID, 'itemActionSettings') || {};
    const globalSettings = item.getFlag(MODULE.ID, 'globalItemSettings') || {};
...
async function setActionSetting(item, actionId, settingPath, value, isInherited = false, parentEnabled = undefined, parentTypes = undefined) {
    const itemActionSettings = item.getFlag(MODULE.ID, 'itemActionSettings') || { actions: [] };
...
class GlobalDamageSettingsForm extends FormApplication {
    ...
    static get defaultOptions() {
        return foundry.utils.mergeObject(super.defaultOptions, {
            ...
            title: game.i18n.localize("NAS.damage.itemSettings.Title"),
            template: `modules/${MODULE.ID}/src/templates/global-damage-settings.html`,
            width: 550,
            height: "auto",
            closeOnSubmit: true
        });
    ...
    async _updateObject(event, formData) {
        ...
        await this.item.setFlag(MODULE.ID, 'globalItemSettings', newStructure);
        
        const itemActionSettings = this.item.getFlag(MODULE.ID, 'itemActionSettings') || { actions: [] };
        ...
        if (this.item.actions && this.item.actions.size > 0) {
            ...
            if (needsUpdate) {
                await this.item.setFlag(MODULE.ID, 'itemActionSettings', itemActionSettings);
            }
        }
...
class ActionDamageSettingsForm extends FormApplication {
    ...
    getData() {
        ...
        attacks.forEach((attack, index) => {
            ...
            if (!attackKey || attackKey === 'undefined' || attackKey === 'null') {
                console.warn("Skipping attack with invalid key during getData:", attack);
                return; 
            }
    ...
    async _updateObject(event, formData) {
        ...
        const currentSettings = this.item.getFlag(MODULE.ID, 'itemActionSettings') || { actions: [] };
        ...
        if (data.attacks) {
            ...
            for (const [attackKey, attackData] of Object.entries(data.attacks)) {
                    if (!attackKey || attackKey === 'undefined' || attackKey === 'null') {
                        console.warn("Skipping invalid attack key:", attackKey);
                        continue;
                    }
        ...
        if (typeof window !== 'undefined' && window._lastItemActionSettings) {
            await this.item.setFlag(MODULE.ID, 'itemActionSettings', window._lastItemActionSettings);
...
class DamageUI {
    ...
    static _onGetItemSheetHeaderButtons(sheet, buttons) {
        buttons.unshift({
            label: game.i18n.localize("NAS.damage.buttons.GlobalSettings"),
            class: "nas-damage-settings",
            icon: "fas fa-bolt",
            onclick: () => {
...
async function initializeDamageFlags(item) {
    let globalItemSettings = item.getFlag(MODULE.ID, 'globalItemSettings') || {};
    globalItemSettings = {
    ...
    await item.setFlag(MODULE.ID, 'globalItemSettings', globalItemSettings);

    let itemActionSettings = item.getFlag(MODULE.ID, 'itemActionSettings');
    let actions = [];
